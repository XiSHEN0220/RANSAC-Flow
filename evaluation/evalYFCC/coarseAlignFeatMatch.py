...
class CoarseAlign:
    def __init__(self, nbScale, nbIter, tolerance, transform, minSize, segId = 1, segFg = True, use_cuda=True, imageNet = True, segNet = True, scaleR = 2):
    ...
    def getCoarse(self, Mt) :
        ...
        with torch.no_grad() :  
            ...
            MtExtend = torch.tensor(MtExtend, device=device).unsqueeze(0).unsqueeze(0)
            
            MtTensor = F.interpolate(input = MtExtend, size = (self.featt.size()[2], self.featt.size()[3]), mode='bilinear')
            MtTensor = (MtTensor > 0.5).float()
            ...
            ## RANSAC
            ones = torch.ones(W1MutualMatch.size(0), device=device)
